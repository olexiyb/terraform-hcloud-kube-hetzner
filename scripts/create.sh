...
if [ ! -e "${folder_path}/kube.tf" ]; then
    curl -sL https://raw.githubusercontent.com/olexiyb/terraform-hcloud-kube-hetzner/dev/kube.tf.example -o "${folder_path}/kube.tf"
else
    echo "kube.tf already exists. Skipping download."
fi

if [ ! -e "${folder_path}/hcloud-microos-snapshots.pkr.hcl" ]; then
    curl -sL https://raw.githubusercontent.com/olexiyb/terraform-hcloud-kube-hetzner/dev/packer-template/hcloud-microos-snapshots.pkr.hcl -o "${folder_path}/hcloud-microos-snapshots.pkr.hcl"
else
